...
testIntervalCorefSeqs <- function(corefOverlaps) {
  ...
  testResults <- corr.test(corefOverlaps[,c("COREF_SEQ_ORDER", "TOKEN_TYPE_OVERLAP")], method="pearson", alpha=ALPHA)
...
testOrdinalCorefSeqs <- function(corefOverlaps) {
  ...
  testResults <- corr.test(corefOverlaps[,c("COREF_SEQ_ORDER", "TOKEN_TYPE_OVERLAP")], method="spearman", alpha=ALPHA)
  print(testResults, short=FALSE, digits=4)
}
