	...
	def __print_session(self, dyad_id: str, coref_groups: GroupCoreferenceChainDatum, outfile):
		for round_id, (game_round, round_instructor_id, round_utts, group_round_corefs) in enumerate(
			...
			for group in TokenGrouping:
				...
				relevant_vocab = self.group_relevant_vocab[group]

				token_metrics = TokenMetrics(relevant_tokens, relevant_vocab)
...
class TokenGroupingDataMappings(object):
	def __init__(self, col_name: str, coref_chain_id_extractor: Callable[[game_events.GameRound], C],
				 ...
				 relevant_vocab_extractor: Callable[[tg.TokenGroupMapping], Iterator[str]]):
		...
		self.relevant_vocab_extractor = relevant_vocab_extractor
...
class TokenGrouping(Enum):
	REFERENT = TokenGroupingDataMappings("REFERENT", _game_round_referent_id, lambda group_tokens: tuple(
		token for tokens in group_tokens.values() for token in tokens),
										 ...
										 lambda token_groups: (token for (token, groups) in token_groups.items() if
															   groups))
...
class TokenMetrics(Generic[C]):
	...
	@staticmethod
	def col_names(group_relevant_vocab: Mapping[TokenGrouping, Iterable[str]]) -> List[str]:
		result = []
		for grouping in TokenGrouping:
			for col_name in TokenMetrics.__COL_NAMES:
				result.append("{}_{}".format(col_name, grouping.value.col_name))
			# Add vocab columns for group
			relevant_vocab = group_relevant_vocab[grouping]
			result.extend("{}_{}".format(token_type, grouping.value.col_name) for token_type in relevant_vocab)
		return result

	def __init__(self, relevant_tokens: Iterable[str], relevant_vocab: Iterable[str]):
		relevant_token_types = frozenset(relevant_tokens)
		self.relevant_tokens_repr = ','.join(sorted(relevant_token_types))
		self.vocab_features = tuple((1 if vocab_entry in relevant_token_types else 0) for vocab_entry in relevant_vocab)
