...
public final class ArgumentSummaryStatistics<A> implements Consumer<A> {
	...
	private A argMax = null;

	private BigDecimal min = null;

	private A argMin = null;

	private BigDecimal sum = BigDecimal.ZERO;

	private final Function<? super A, BigDecimal> argValGetter;

	public ArgumentSummaryStatistics(final Function<? super A, BigDecimal> argValGetter) {
		this.argValGetter = argValGetter;
	}
	 ...
	 */
	@Override
	public void accept(final A arg) {
		count = count.add(BigDecimal.ONE);
		final BigDecimal argVal = argValGetter.apply(arg);
		sum = sum.add(argVal);
		final BigDecimal oldMin = this.min;
		min = minNullable(min, argVal);
		if (oldMin.compareTo(min) > 0) {
			this.argMin = arg;
		}
		final BigDecimal oldMax = this.max;
		max = maxNullable(max, argVal);
		if (oldMax.compareTo(max) < 0) {
			this.argMax = arg;
		}
	}
	 ...
	 */
	public void combine(final ArgumentSummaryStatistics<A> other) {
		...
		if (oldMin.compareTo(min) > 0) {
			this.argMin = other.argMin;
		}

		final BigDecimal oldMax = this.max;
		max = maxNullable(max, other.max);
		if (oldMax.compareTo(max) < 0) {
			this.argMax = other.argMax;
		}
	}
	...
	public String toString() {
		return String.format("%s{count=%d, sum=%d, min=%d, argMin=%s, average=%f, max=%d, argMax=%s}",
				this.getClass().getSimpleName(), getCount(), getSum(), getMin(), getArgMin(), getAverage(), getMax(),
				getArgMax());
	}
}
