 ...
 *
 */
public final class GameStateEventReplayer extends Observable {

	private static final Logger LOGGER = LoggerFactory.getLogger(GameStateEventReplayer.class);

	private static Stream<Object> createNotificationArgs(final Event event) {
		...
		case COMPLETED_TURN_REQUEST: {
			...
			result = Stream.of(move, submittingPlayerId);
			break;
		}
		...
		case SELECTION_REQUEST: {
			final String selectingPlayerId = event.getString(GameManagementEvent.Attribute.PLAYER_ID.toString());
			final Selection selection = (Selection) event.get(GameManagementEvent.Attribute.SELECTION.toString());
			result = Stream.of(selection, selectingPlayerId);
			break;
		}
		default: {
			throw new IllegalArgumentException(
					String.format("Cannot create notification args for event type \"%s\".", gameEventType));
		}
		}
		return result;
	}
	...
	private static Event createUndoEvent(final Event event, final String time) {
		...
		result.setTime(time);
		{
			final String key = GameManagementEvent.Attribute.PLAYER_ID.toString();
			final String submittingPlayerId = event.getString(key);
			result.put(key, submittingPlayerId);
		}
		...
		case COMPLETED_TURN_REQUEST: {
			{
				final String key = GameManagementEvent.Attribute.MOVE.toString();
	...
	public GameStateEventReplayer(final List<Event> events, final Timestamp initialTime) {
	...
	private void notifyEvents(final List<Event> events) {
		final Stream<Object> args = events.stream().flatMap(GameStateEventReplayer::createNotificationArgs);
		final Event lastEvent = events.listIterator(events.size()).previous();
		LOGGER.debug("Last notified event: {}", lastEvent);
		setChanged();
